...
METHOD(vprepproc_init) {
    struct vprep_s *state = malloc(sizeof *state);

    const int32_t error = vprep_init(state);
...
METHOD(vpreproc_execute) {
    ASSERT_ARGC(3)
    struct vprep_s *state;
    ASSERT_EXTERNAL(args[0], state)
    ASSERT_STRING(args[1], p)
    ASSERT_STRING(args[2], endp)

    const int32_t error = vprep_execute(state, p, endp);
...
napi_value Init(napi_env env, napi_value exports) {
    DECLARE_NAPI_METHOD("init", vprepproc_init)
    DECLARE_NAPI_METHOD("execute", vpreproc_execute)
    return exports;
}
